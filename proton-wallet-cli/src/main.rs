...
fn add_account(
    ...
    let mut lock = wallet.lock().unwrap();
    let derivation_path = lock.add_account(script_type, account_index, ());
...
async fn get_account_transactions(
    ...
    account_lock
        ...
        .for_each(|simple_tx| {
            println!(
                "txid: {:?} | time {} | value: {} sats | fees: {} ",
                simple_tx.txid,
                match simple_tx.time {
                    TransactionTime::Confirmed { confirmation_time } => format!("Confirmed - {}", confirmation_time),
                    TransactionTime::Unconfirmed { .. } => format!("Unconfirmed"),
                },
                simple_tx.value,
