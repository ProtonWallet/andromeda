...
pub use bdk::bitcoin::bip32::DerivationPath;
...
impl From<BdkDescriptorSecretKey> for DescriptorSecretKey {
    fn from(value: BdkDescriptorSecretKey) -> DescriptorSecretKey {
        Self { inner: value }
    }
}

impl DescriptorSecretKey {
    pub fn new(network: Network, bip39_mnemonic: Arc<Mnemonic>, bip38_passphrase: Option<String>) -> Self {
        let mnemonic = bip39_mnemonic.inner();
        let xkey: ExtendedKey = (mnemonic, bip38_passphrase).into_extended_key().unwrap();
    ...
    pub fn derive(&self, path: DerivationPath) -> Result<Arc<Self>, BdkError> {
    ...
    pub fn extend(&self, path: DerivationPath) -> Result<Arc<Self>, BdkError> {
        let descriptor_secret_key = &self.inner;
...
pub trait Deriveable {
    fn derive(&self, path: DerivationPath) -> Result<Arc<Self>, BdkError>;
}

impl Deriveable for BdkDescriptorSecretKey {
    fn derive(&self, path: DerivationPath) -> Result<Arc<Self>, BdkError> {
        let secp = Secp256k1::new();
...
pub trait Extendable {
    fn extend(&self, path: DerivationPath) -> Result<Arc<Self>, BdkError>;
}

impl Extendable for BdkDescriptorSecretKey {
    fn extend(&self, path: DerivationPath) -> Result<Arc<Self>, BdkError> {
        match self {
            BdkDescriptorSecretKey::XPrv(descriptor_x_key) => {
                let extended_path = descriptor_x_key.derivation_path.extend(path);
...
pub fn new_master_private_key(mnemonic_str: &str, passphrase: Option<String>) -> ExtendedPrivKey {
    ...
    masterxkey.into_xprv(Network::Testnet).unwrap()
}
